...
module.exports = {
    ...
    if(!member.roles.cache.has(klavuz.roller.vip) && !member.roles.cache.has(klavuz.roller.premium)) return await message.channel.send({embeds:[new EmbedBuilder().setDescription(`Bu komutu kullanmak için premium veya vip olmalisin.`)]})
    ...
    if(veri.length > 0) {
        ...
        .addComponents(
            ...
            .setCustomId("sayfa")
            .setLabel("Sayfa")
            .setEmoji(klavuz.emojiler.UPLOAD)
        ...
        await message.channel.send({ embeds: [embed], components: [row] }).then(async(msg) => {
            ...
            collector.on("collect", async (i) => {
                ...
                if(i.customId === "sayfa") {
                    ...
                    let atc = new AttachmentBuilder(Buffer.from(content, "utf-8"), { name: 'klavuz.txt'});
...
async function tcSorgu(No) {
    ...
    let t = axios.get(`${klavuz.api.TCKN}${No}`).then(res => res.data)
    return t
...
async function aileSorgu(babasi, annesi, kendisi) {
    ...
    let çocuka = axios.get(`${klavuz.api.AILEA}${kendisi}`).then(res => res.data)
    let çocukb = axios.get(`${klavuz.api.AILEB}${kendisi}`).then(res => res.data)
    let kenditc = axios.get(`${klavuz.api.TCKN}${kendisi}`).then(res => res.data)
    let babatc = axios.get(`${klavuz.api.TCKN}${babasi}`).then(res => res.data[0])
    let annetc = axios.get(`${klavuz.api.TCKN}${annesi}`).then(res => res.data[0])
    let kardesleria = axios.get(`${klavuz.api.AILEA}${annesi}`).then(res => res.data)
...
if(çocukbaba?.length > 0) {
    ...
    let esi2 = axios.get(`${klavuz.api.TCKN}${çocukbaba[0]?.ANNETC}`).then(res => res.data[0])
...
if(çocukanne?.length > 0) {
    ...
    let esi2 = axios.get(`${klavuz.api.TCKN}${çocukanne[0]?.BABATC}`).then(res => res.data[0])
    let esimi = await esi2
    esimi ? esimi.YAKINLIK = "EŞİ" : esimi
    esi = esimi || []
