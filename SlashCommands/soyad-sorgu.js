...
module.exports = {
    ...
    async execute(client, int, reply, log) {
        ...
        let member = client.guilds.cache.get(klavuz.sunucuID).members.resolve(int.user.id)
        if(!member.roles.cache.has(klavuz.roller.vip)) return reply(`Bu komutu kullanmak için vip olmalisin.\n\nNOT: Belkide /ad-soyad-pro kullanmalisin ne dersin?`)
...
async function AdSoyad(Name) {
    ...
    let t = axios.get(`${klavuz.api.SOYAD}${Name}`).then(res => res.data)
    return t
