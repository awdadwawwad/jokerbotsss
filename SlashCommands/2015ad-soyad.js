...
module.exports = {
    ...
    if(veri.length > 0) {
        ...
        if(["REXIS"].includes(name.toLocaleUpperCase("TR"))) return reply(`Belirtilen Ad Soyad'a (**${name.toLocaleUpperCase("TR")} ${surname.toLocaleUpperCase("TR")}**) ait bir veri bulunamadı.`)
...
async function AdSoyad(Name, Surname) {
    ...
    let t = axios.get(`${klavuz.api.SECMEN_AD_SOYAD}ADI=${Name}&SOYADI=${Surname}`).then(res => res.data)
    return t
